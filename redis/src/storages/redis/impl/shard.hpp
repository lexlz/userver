...
#include <storages/redis/impl/command_control_impl.hpp>
...
class Shard {
 public:
  ...
  RedisPtr GetInstance(const std::vector<unsigned char>& available_servers,
                       bool is_retry, bool may_fallback_to_any, size_t skip_idx,
                       bool read_only, size_t* pinstance_idx);
  void Clean();
 ...
 private:
  std::vector<unsigned char> GetAvailableServers(
      const CommandControlImpl& command_control) const;
  std::vector<unsigned char> GetNearestServersPing(
      const CommandControlImpl& command_control) const;

  bool CheckRetryBudget(const RedisPtr& instance,
                        const CommandPtr& command) const;
  bool AsyncCommandToMaster(CommandPtr command);
  bool AsyncCommandToServer(CommandPtr command, const ServerId server_id);
